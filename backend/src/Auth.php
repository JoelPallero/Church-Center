...
class Auth
{
    ...
    private $activity;
    private $bypassLogin = false; // Disable bypass to use real DB login
    ...
    private function loadSecuritySettings()
    {
        ...
        try {
            ...
            // Log fallback
            $this->jwtSecret = 'default_fallback';
    ...
    public function login($email, $password, $ipAddress = null, $userAgent = null)
    {
        ...
        if ($this->bypassLogin) {
            return [
                ...
                'token' => 'BYPASS_TOKEN_DEV_MODE',
                'refresh_token' => 'BYPASS_REFRESH'
            ];
        }
        ...
        $tokenData = [
            'uid' => $user['user_id'],
            'mid' => $user['member_id'],
            'cid' => $user['church_id'], // Null for Master
            'role' => $user['role_name'],
        ...
        Debug::security("FULL_LOGIN_SUCCESS | User: {$user['name']} (ID: {$user['member_id']}) | Email: {$email} | Role: {$user['role_name']} | CID: " . ($user['church_id'] ?? 'NULL'));
