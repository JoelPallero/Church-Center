...
-- ============================================================
-- Consolidates: Core Schema + Calendar + Guest Flow + Permissions + Seeds
...
DROP TABLE IF EXISTS meeting_assignments; -- Legacy table
DROP TABLE IF EXISTS playlist_items;
DROP TABLE IF EXISTS playlists;
DROP TABLE IF EXISTS meeting; -- Legacy table
...
-- 4. MUSIC DOMAIN (LIBRARY REFERENCES)
-- This table tracks which songs from MusicCenter belong to which church/user
CREATE TABLE songs (
    id INT AUTO_INCREMENT PRIMARY KEY,
    master_song_id INT NOT NULL, -- Link to u593735198_musiccenter
    church_id INT NOT NULL,
    created_by INT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
(1, 'master', 'Super Administrador', 'Control total del SaaS', 0, TRUE),
(2, 'pastor', 'Pastor / Administrador local', 'Control total de la congregación', 10, TRUE),
(3, 'leader', 'Líder de Ministerio', 'Gestión de equipos y música', 30, TRUE),
(4, 'coordinator', 'Coordinador', 'Gestión operativa del área', 40, TRUE),
(5, 'member', 'Miembro / Voluntario', 'Acceso básico', 100, TRUE),
(6, 'guest', 'Invitado', 'Usuario recién registrado sin equipo asignado', 200, TRUE);
...
-- Areas
INSERT INTO area (id, church_id, name) VALUES (1, 1, 'Alabanza');

-- Groups (linked to Areas)
INSERT INTO `group` (church_id, area_id, name, description) VALUES (1, 1, 'Banda de Domingo', 'Música en vivo');

-- DEFAULT SONG (Reference 1: Cantaré de ti)
INSERT INTO songs (id, master_song_id, church_id, created_by) VALUES (1, 1, 1, 1);

-- ADMIN USER (Clave: Master2024!)
INSERT INTO member (id, church_id, name, email, role_id, status_id) VALUES (1, 1, 'Administrador MSM', 'admin@system.master', 1, 1);
