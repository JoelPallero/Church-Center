            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead>
                    <tr style={{ borderBottom: '1px solid var(--color-border-subtle)', backgroundColor: 'rgba(0,0,0,0.02)' }}>
                        <th style={{ padding: '10px 16px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--color-ui-text-soft)' }}>Título</th>
                        <th style={{ padding: '10px 16px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--color-ui-text-soft)' }}>Artista</th>
                        <th style={{ padding: '10px 16px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--color-ui-text-soft)', textAlign: 'center' }}>Tono</th>
                        <th style={{ padding: '10px 16px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--color-ui-text-soft)', textAlign: 'center' }}>BPM</th>
                        <th style={{ padding: '10px 16px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--color-ui-text-soft)', textAlign: 'right' }}>Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    {songs.map(song => (
                        ...
                        >
                            <td style={{ padding: '8px 12px', width: '35%' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    ...
                                    <div>
                                        <div style={{ fontWeight: 600, color: 'var(--color-ui-text)', fontSize: '13px', lineHeight: 1.2 }}>{song.title}</div>
                                        {song.isAssigned && (
                                            <div style={{ fontSize: '10px', color: 'var(--color-brand-blue)', fontWeight: 600 }}>{song.singerName}</div>
                                        )}
                                        {isMaster && song.churchName && (
                                            <div style={{ fontSize: '9px', color: 'var(--color-ui-text-soft)' }}>{song.churchName}</div>
                                        )}
                                    </div>
                                </div>
                            </td>
                            <td style={{ padding: '8px 12px', color: 'var(--color-ui-text-soft)', fontSize: '12px', width: '25%' }}>{song.artist}</td>
                            ...
                            <td style={{ padding: '8px 12px', textAlign: 'center', width: '10%' }}>
                                <span style={{ fontWeight: 700, fontSize: '12px', color: 'var(--color-ui-text)' }}>
                                    {song.tempo} <small style={{ fontWeight: 400, opacity: 0.6, fontSize: '10px' }}>BPM</small>
                                </span>
                            </td>
