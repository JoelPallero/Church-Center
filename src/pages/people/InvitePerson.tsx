        ...
        try {
            if (isBulk) {
                ...
                if (emailList.length === 0) {
                    throw new Error('Por favor, ingresa al menos un correo válido.');
                }
                ...
                if (result.success > 0) {
                    setStatus({
                        type: 'success',
                        message: `Se enviaron ${result.success} invitaciones correctamente.${result.failed > 0 ? ` (${result.failed} fallaron)` : ''}`
                    });
                    setBulkEmails('');
                } else {
                    setStatus({ type: 'error', message: 'No se pudo enviar ninguna invitación.' });
                }
            } else {
                ...
                if (success) {
                    ...
                    setStatus({ type: 'error', message: 'Error al enviar la invitación.' });
                }
            }
        } catch (err: any) {
    ...
    return (
        <div>
            <header style={{ marginBottom: '24px' }}>
                ...
                <p className="text-body" style={{ color: '#6B7280' }}>Envía una invitación por correo para unirse al equipo.</p>
            </header>
            ...
            <Card style={{ padding: '24px', maxWidth: '500px' }}>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    {!isBulk ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            <label className="text-overline" style={{ color: '#6B7280' }}>Correos electrónicos</label>
                            <textarea
                                placeholder={"Ingresa los correos separados por coma o uno por línea..."}
                                value={bulkEmails}
                            ...
                            <p className="text-overline" style={{ color: '#9CA3AF', marginTop: '4px' }}>
                                Las invitaciones masivas se enviarán con el rol de "Miembro" por defecto.
                            </p>
                        </div>
                    )}
                    ...
                    <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
                        ...
                        <Button
                            type="submit"
                            label={isLoading ? 'Enviando...' : t('people.invite')}
                            variant="primary"
                            disabled={isLoading}
                            style={{ flex: 1 }}
                        />
