 ...
 */
$possiblePaths = [
    __DIR__ . '/../../backend/src/Auth.php',          // Local (public/api -> root)
    __DIR__ . '/../../msm_logic/src/Auth.php',        // Pro (Sibling of public_html)
    __DIR__ . '/../../../msm_logic/src/Auth.php',     // Pro (Root of account)
    __DIR__ . '/../../backend/src/Auth.php'           // Fallback
];
...
if (!$logicPath) {
    ...
    echo json_encode(["message" => "Critical Error: Core logic not found.", "debug_checked" => $possiblePaths]);
    exit();
}
        ...
        if ($token) {
            ...
            if ($payload) {
                ...
                echo json_encode([
                    "success" => true,
                    "user" => $user,
                    "permissions" => $auth->getUserPermissions($payload['mid'])
                ]);
            } else {
                http_response_code(401);
