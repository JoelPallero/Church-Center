# Tareas Pendientes

## Canciones
- [ ] **Idiomas en Canciones**: Implementar funcionalidad para cambiar el idioma de una canción manualmente. Cada canción podrá tener múltiples versiones (idiomas) y se podrá alternar entre ellas.
    - *Nota*: Por ahora se guardan y muestran tal como se escribieron. Las canciones **no** se traducen automáticamente.

## Sistema
- [x] Persistencia de Tema y Lenguaje por defecto en la base de datos.
- [x] Corregir textos en inglés que deberían estar en español.


## Horario
- [x] La hora debe ser la de Buenos Aires.
- [x] El menu de perfil está por abajo del blur.
- [x] textos con nombre clave interno de código (e.g. auth.forgotPassword) en lugar de texto plano.
- [x] En el header figura branding contextual (Pastoral Hub, Ministry Hub, Tech Hub, Diaconos Hub, Social Media Hub) según la ruta y el rol.
- [ ] El sistemas de notificaciones que se pueda editar el texto del mail y usar hasta 3 plantillas diferentes para los diferentes tipos de notificaciones.
- [x] Añadir por defecto 2 áreas: Ujieres y Alabanza, cuando se crea una iglesia nueva.
- [x] Para los ujieres: Implementado sistema de conteo y visitantes (Frontend + Backend).
- [x] La página de reportes no funciona: Corregido error de redirección ybackend implementado.
## Diseño e Identidad
- [x] **Rebranding Global**: Cambiar "MinistryHub" por "Church Center" en todo el sistema.
- [ ] **Página Institucional**: Implementar la nueva página de aterrizaje institucional (basada en el diseño de Stitch) que describa el ecosistema de Hubs.
- [ ] **Unificación de Idiomas**: Asegurar que la página institucional y todo el sistema soporte Español, Inglés y Portugués.
- [x] **Estructura de Hubs**: Reflejada la división de servicios independientes pero integrados:
    - **Ministry Hub**: Alabanza y música.
    - **Sound Hub**: Ingeniería de sonido y audio.
    - **Multimedia Hub**: Proyección de letras, contenido visual y video.
    - **Diaconos Hub**: Ujieres y logística.
    - **Social Media Hub**: Redes sociales, CMs y transmisiones.
    - **Pastoral Hub**: Panel de control total, reportes en tiempo real, estadísticas, calendarios y gestión de equipos para el Pastor.
- [ ] usar httpOnly cookies
- [ ] en mobile, no aparecen todas las opciones en el menu del header.
- [ ] El ancho de mobile, cambió. Hay que corregirlo.

